...
contract ProviderController is Ownable {
    ...
    struct Provider {
        address owner;
        uint32 nextWithdrawal;
        uint256 fee; // fee is the cost in token units that the provider charges to subscribers per month
        uint256 balance; // the provider balance is stored in the contract. Equals to total amount withdrawn by provider
    }
    ...
    function registerProvider(bytes calldata registerKey, uint256 fee) external returns (uint64 id) {
        ...
        providers[id] = Provider({owner: msg.sender, balance: 0, fee: fee, nextWithdrawal: uint32(block.timestamp)});
    ...
    function removeProvider(uint64 providerId) external onlyProviderOwner(providerId) {
        ...
        // improve gas cost
        (uint256 currentBalance,) = calculateProviderEarnings(providerId);

        uint64[] memory subs = providerSubscribers[providerId];
        for (uint i = 0; i < subs.length; i++) {
            uint64 subscriberId = subs[i];
            removeProviderFromSubscriberMap(providerId, subscriberId);
        }

        providersLength--;
        delete providers[providerId];
        delete providerSubscribers[providerId];
    ...
    function withdrawProviderEarnings(uint64 providerId) public onlyProviderOwner(providerId) {
        ...
        // check that provider can withdraw only once in 30 days
        if(block.timestamp < providers[providerId].nextWithdrawal) revert WithdrawalNotAvailableYet();
        // IMPORTANT: before withdrawing, the amount earned from subscribers needs to be calculated
        (uint256 amount, uint32 months) = calculateProviderEarnings(providerId);
        providers[providerId].balance += amount; // balance contains total amount withdrawn
        providers[providerId].nextWithdrawal = providers[providerId].nextWithdrawal + PAYMENT_FREQUENCY * months;
        if(amount > 0)
    ...
    function pauseSubscription(uint64 subscriberId) external {
        ...
        for (uint i; i < providerIds.length; i++) {
            ...
            removeSubscriber(providerId, subscriberId);
    ...
    // private functions
    function calculateProviderEarnings(uint64 providerId) private view returns (uint256 earnings, uint32 months) {
        ...
        uint256 earnings;
        uint32 monthsSinceLastWithdrawal;
        if(block.timestamp > providers[providerId].nextWithdrawal) {
            monthsSinceLastWithdrawal = ((uint32(block.timestamp) - providers[providerId].nextWithdrawal) / PAYMENT_FREQUENCY) + 1;
            earnings = providers[providerId].fee * providerSubscribers[providerId].length * monthsSinceLastWithdrawal;
        }
        return (earnings, monthsSinceLastWithdrawal);
    }
    ...
    function removeProviderFromSubscriberMap(uint64 providerId, uint64 subscriberId) private {
        uint64[] storage provs = subscriberProviders[subscriberId];
        for (uint i = 0; i < provs.length; i++) {
            if (provs[i] == providerId) {
    ...
    function removeSubscriber(uint64 providerId, uint64 subscriberId) private {
        uint64[] storage subs = providerSubscribers[providerId];
        for (uint i; i < subs.length; i++) {
            if (subs[i] == subscriberId) {
                subs[i] = subs[subs.length - 1]; // Swap with the last item
                subs.pop(); // Remove the last item
                break;
            }
        }
    }
    ...
    function getProviderState(uint64 providerId) external view providerExists(providerId) returns (
        ...
        return (providerSubscribers[providerId].length, provider.fee, provider.owner, provider.balance, isProviderActive(providerId));
    }
    ...
    function getSubscriberState(uint64 _subscriberId) external view subscriberExists(subscriberId) returns (
        address owner, 
        uint256 balance, 
        Plan plan, 
        bool isPaused
    ...
    function getSubscriberLiveBalance(uint64 subscriberId) external view subscriberExists(subscriberId) returns (uint256) {
        ...
        // provider earnings are calculated as monthsSinceSubscription * fees
        uint256 providerEarnings = ((endDate - subscriber.createdDate) / PAYMENT_FREQUENCY) * fees ;
